...
public class SpatiaLiteAnnotationProcessor extends AnnotationProcessor implements IAnnotationProcessor, Cloneable
//===============================================================================================================
{
   static final private String TAG = SpatiaLiteAnnotationProcessor.class.getName();
   ...
   public SpatiaLiteAnnotationProcessor() {  }

   public SpatiaLiteAnnotationProcessor(Context context) { super(context); }

   @Override
   public ISpatialSource createAnnotated(String name, Context context, ActiveObject activeObject, Object instance)
   //-------------------------------------------------------------------------------------------------------------
   {
      final Class<?> C = instance.getClass();
      String annotatedName = " (" + C.getName() + ")";
      SpatiaLiteSource source = C.getAnnotation(SpatiaLiteSource.class);
      ...
      SpatiaLiteDataSource dataSource = new SpatiaLiteDataSource(sourceName, context, activeObject, this);
      ...
      for (Field field : fields)
      {
         ...
         {
            String v = getAnnotatedField(field, DatabaseFile.class, false, instance, String.class, File.class);
            if (v != null)
            {
               File f = new File(v);
               if (! f.exists())
                  throw new RuntimeException("Could not find Spatialite database at " + f);
               try { dataSource.open(f); } catch (Exception _e) { throw new RuntimeException("Could not open Spatialite database at " + f, _e); }
            }
         }
      }
